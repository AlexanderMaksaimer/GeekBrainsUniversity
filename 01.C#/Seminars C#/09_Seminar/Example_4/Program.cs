...
{//Метод ввода и проверки на число
    ...
    while (readlineFromStrToInt)
    {
        ...
        {//Можно было этого и не делать, но хотелось проверить
            case 'A':
...
{//Метод возведения в степень используя Math.Pow - не используется
...
{//Я очень не люблю большие цифры когда они пишутся слитно, пришлось выдумывать метод добавления проблемлов.
    string numberSTR = Convert.ToString(number);
    string result = String.Empty;
